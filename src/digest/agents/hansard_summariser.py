...
from digest.sources.hansard.constants import Persona, Source, PERSONA_PROMPTS
from digest.sources.hansard.main import get_hansard_data_extractor
...
hansard_data_extractor = get_hansard_data_extractor()
...
async def get_hansard_summary(
    dt: date, source: Source = Source.DEBATES
) -> dict[Persona, str]:
    debate = await hansard_data_extractor.extract_day(dt, source)
...
def summarise(
    ...
    source: Annotated[
        Source, Option(help="Source of debate to summarise.")
    ] = Source.DEBATES,
